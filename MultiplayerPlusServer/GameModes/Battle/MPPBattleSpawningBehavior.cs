...
namespace MultiplayerPlusServer.GameModes.Battle
{
    ...
    public class MPPBattleSpawningBehavior : SpawningBehaviorBase
    {
        ...
        {
            if (!_enforcedSpawnTimers.Any((KeyValuePair<MissionPeer, TaleWorlds.Core.Timer> pair) => pair.Key == peer))
            {
                _enforcedSpawnTimers.Add(new KeyValuePair<MissionPeer, TaleWorlds.Core.Timer>(peer, new TaleWorlds.Core.Timer(base.Mission.CurrentTime, durationInSeconds)));
        ...
        {
            KeyValuePair<MissionPeer, TaleWorlds.Core.Timer> keyValuePair = _enforcedSpawnTimers.FirstOrDefault((KeyValuePair<MissionPeer, TaleWorlds.Core.Timer> pr) => pr.Key == peer);
            ...
            {
                _enforcedSpawnTimers.RemoveAll((KeyValuePair<MissionPeer, TaleWorlds.Core.Timer> p) => p.Key == peer);
                Debug.Print("EST for " + peer.Name + " is no longer valid (spawned already).", 0, Debug.DebugColor.Yellow, 64uL);
                return false;
            }

            TaleWorlds.Core.Timer value = keyValuePair.Value;
